...
const UserRoutes = async (fastify) => {
    fastify
        ...
        .delete("", {
        schema: user_request_schema_1.deleteUserProfile,
    }, async (req, res) => {
        try {
            const user = (0, auth_util_1.getUserIdFromRequestHeader)(req);
            await (0, user_usecase_1.deleteUserProfileUseCase)(user.userId);
            (0, response_1.createSuccessResponse)(res, "User deleted successfully!");
        }
        catch (error) {
            const message = error.message || "An unexpected error occurred";
            const statusCode = error.status || 500;
